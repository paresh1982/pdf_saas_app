 ...
 */
const EXECUTIVE_REPORT_PROMPT = `You are the Lead Financial Auditor for NexGen DocJockey.
Your task is to synthesize a high-fidelity Executive Summary based on the provided document context and chat history.

STRUCTURE:
1. Executive Summary: High-level purpose and key findings.
2. Data Integrity Audit: Verification of data points (Dates, Amounts, Totals).
3. Strategic Insights: Potential risks or opportunities identified in the documents.
4. Final Verdict: Professional recommendation.

FORMATTING:
- Use Markdown for the content.
- Use bold headers.
- Include tables for financial data if relevant.
- Keep the tone enterprise-grade and authoritative.
`;
...
<head>
    <meta charset="utf-8">
    <title>DocJockey Executive Audit Report</title>
    ...
    <style>
        ...
        .meta { color: var(--secondary); font-variant: all-small-caps; letter-spacing: 0.2em; font-weight: 700; margin-top: 5px; }
        
        table { width: 100%; border-collapse: collapse; margin: 30px 0; background: rgba(255,255,255,0.02); border-radius: 12px; overflow: hidden; }
        th { background: rgba(255,255,255,0.05); color: var(--secondary); text-align: left; padding: 15px; font-size: 11px; text-transform: uppercase; letter-spacing: 0.1em; }
        td { padding: 15px; border-bottom: 1px solid rgba(255,255,255,0.05); font-size: 14px; }
        
        .footer { margin-top: 60px; padding-top: 30px; border-top: 1px solid rgba(255,255,255,0.1); font-size: 10px; color: rgba(255,255,255,0.3); text-transform: uppercase; letter-spacing: 0.2em; text-align: center; }
        
        @media print {
            body { background: white; color: black; padding: 0; }
            .container { box-shadow: none; border: none; max-width: 100%; padding: 20px; }
            h1, h2, h3 { color: black; }
            .logo { background: black; border: 1px solid black; }
        }
    </style>
</head>
<body>
    <div class="container">
        <div class="header">
            ...
            <div>
                ...
                <div class="meta">NexGen Intelligence Engine • ${new Date().toLocaleDateString()}</div>
            </div>
        </div>
